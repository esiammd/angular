...
export class CountryService {
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError((error) => {
          ...
          return throwError(
            () => new Error(`No se pudo obtener paíse con el query ${query}`)
          );
        })
      );
  }
  ...
  searchByCountry(query: string): Observable<Country[]> {
    ...
    return this.http
      .get<RESTCountry[]>(`${API_URL}/name/${lowerCaseQuery}`)
      .pipe(
        map((restCountries) =>
          CountryMapper.mapRestCountryArrayToCountryArray(restCountries)
        ),
  ...
  searchByRegion(region: Region): Observable<Country[]> {
    ...
    return this.http.get<RESTCountry[]>(`${API_URL}/region/${region}`).pipe(
      ...
      catchError((error) => {
        console.log('Error fetching', error);
        return throwError(
          () => new Error(`No se pudo obtener paíse con el query ${region}`)
        );
      })
    );
  }
  ...
  searchCountryByAlphaCode(code: string): Observable<Country | undefined> {
    return this.http.get<RESTCountry[]>(`${API_URL}/alpha/${code}`).pipe(
      ...
      catchError((error) => {
        console.log('Error fetching', error);
        return throwError(
          () => new Error(`No se pudo obtener paíse con el código ${code}`)
        );
      })
    );
  }
